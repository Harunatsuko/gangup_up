...
#                     w, h = imagesize.get(os.path.join(self.root_directory, "images", self.annot_folder, f"{img}.jpg"))
                    ...
                    print('NO SUCH IMAGE!')
                    continue
                annotations.append([str(img) + ".npy", h, w])
        ...
        print('data length', self.len_ann)
        if rect_training:
...
#         img = np.array(Image.open(os.path.join(self.root_directory, self.fname, img_name)).convert("RGB"))
        img = np.load(os.path.join(self.root_directory, self.fname, img_name))
...
#         img = img.transpose((2, 0, 1))
        ...
        img[0:3, :, :] = np.ceil(img[0:3, :, :]*np.random.randint(5,15)/10)
        img[img > 255.0] = 255.0
...
#                 labels = np.roll(labels, axis=1, shift=1)
        ...
        # ?where is batch_size?
        targets = [torch.zeros((self.num_anchors // 3, int(640/S),
                                int(640/S), 6))
                   for S in self.S]
